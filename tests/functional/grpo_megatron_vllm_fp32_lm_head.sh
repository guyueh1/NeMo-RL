...
# Qwen2.5 keeps this L1 aligned with the existing Megatron+vLLM GRPO smoke
# while still exercising the fp32_lm_head config and launch path.
cd $PROJECT_ROOT
...
grep -q "\[fp32_lm_head\] output layer will compute logits in fp32" $RUN_LOG
grep -Eq "Applied NemotronH fp32 LM head source patch|NemotronH fp32 LM head patch already present" $RUN_LOG
