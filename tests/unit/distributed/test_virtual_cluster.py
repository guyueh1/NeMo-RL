...
def test_sort_bundle_indices_fallback_by_node_id_gpu_id():
    ...
    assert result == [
        ...
        2,
        0,
    ]  # bundle 1: node-a gpu0, bundle 2: node-a gpu1, bundle 0: node-b gpu2
...
def test_sort_bundle_indices_segment_requires_gpus_per_node():
    ...
    with pytest.raises(ValueError, match="gpus_per_node is required"):
        _sort_bundle_indices_by_topology(
            bundle_data, segment_size=2, gpus_per_node=None
        )
...
def test_sort_bundle_indices_segment_complete_domain():
    ...
    result = _sort_bundle_indices_by_topology(
        bundle_data, segment_size=2, gpus_per_node=4
    )
    assert len(result) == 8
...
def test_sort_bundle_indices_segment_multi_domain():
    ...
    result = _sort_bundle_indices_by_topology(
        bundle_data, segment_size=2, gpus_per_node=2
    )
...
def test_select_segment_nodes_basic():
    ...
    training, remaining = select_segment_nodes(
        topology, segment_size=2, num_training_nodes=4
    )
...
def test_select_segment_nodes_partial_selection():
    ...
    training, remaining = select_segment_nodes(
        topology, segment_size=2, num_training_nodes=2
    )
